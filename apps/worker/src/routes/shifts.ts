...
async function handleCreateShiftRequest(request: Request, env: Env): Promise<Response> {
  ...
  const monday = new Date(`${weekStartDate}T00:00:00+09:00`).getDay()
  if (monday !== 1) {
...
async function handlePatchShift(request: Request, env: Env, shiftId: string): Promise<Response> {
  ...
  await env.DB
    .prepare(`
      UPDATE shifts
      SET status = ?, start_time = ?, end_time = ?, completion_note = ?, completion_photo_urls = ?, updated_at = datetime('now')
      WHERE id = ?
    `)
    .bind(status, startTime, endTime, completionNote, completionPhotoUrls, shiftId)
    .run()
